...
class ProductControllerIntegrationTest {
    ...
    void setup() {
        ...
        p1.setPrice(new BigDecimal("19.99")); // requerido
        p1.setStock(50); // requerido si es NOT NULL
        p1.setActive(true);
    ...
    void detail_existing_returns200_andBody() throws Exception {
        mockMvc.perform(get("/api/products/{id}", activeProduct.getId()))
    ...
    void list_withNameFilter_returnsFilteredPage() throws Exception {
        mockMvc.perform(get("/api/products").param("name", "Camiseta"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[0].name",
                        Matchers.containsString("Camiseta")));
    }
    ...
    void update_existing_returns200_andUpdatedBody() throws Exception {
        mockMvc.perform(put("/api/products/{id}", activeProduct.getId())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void update_whenChangingSkuToExisting_returns409_or400() throws Exception {
        // Intentar poner el SKU de inactiveProduct a activeProduct
        mockMvc.perform(put("/api/products/{id}", activeProduct.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(productJson("Nombre", "SKU-002", "desc", "29.99", 10, true)))
                .andExpect(status().isConflict()); // si tu handler mapea a 409
        // .andExpect(status().isBadRequest()); // si lo mapeaste a 400
    }

    @Test
    void update_notFound_returns404() throws Exception {
        // Body VÁLIDO para que no caiga 400
        mockMvc.perform(put("/api/products/{id}", 987654321L)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void delete_existing_returns204_andSetsInactive() throws Exception {
        mockMvc.perform(delete("/api/products/{id}", activeProduct.getId()))
